...
struct OrderJar
{
    ...
    public:
        ...
        // Core
        bool isValid(int i_ticket) { return(true); } // Test if the order belongs to this OrderJar.
        int collect(void); // Collect order tickets.
        bool needUpdate(void); // Check if need re-collect.
        void build(void); // Build Order objects.
        void destroy(void); // Destroy Order objects.
        int fetchOrders(Order* &o_orders[]); // Overload to fetch the wanted orders object array.
        Order* fetchOrder(int i_idx); // Simple override is OK because when need use, the external will address the correct class.
        // Value
